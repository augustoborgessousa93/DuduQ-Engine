...
for (let module = 1; module <= 6; module += 1) {
  ...
  for (const viewport of viewports) {
    ...
    try {
      ...
      check(frameMetrics.textLength > 0, `M${mm}/${viewport.name}: mecânica sem conteúdo semântico`);
...
await browser.close();
fs.writeFileSync(path.join(outDir, "report-base.json"), JSON.stringify({ base, cases: report }, null, 2));

if (failures.length) {
  console.error("DUDUQ YEAR2 VISUAL/FUNCTIONAL BASE QA: FAIL");
  for (const failure of failures) console.error(`${failure.module}/${failure.viewport}: ${failure.error}`);
  process.exit(1);
}

console.log("DUDUQ YEAR2 VISUAL/FUNCTIONAL BASE QA: PASS");
