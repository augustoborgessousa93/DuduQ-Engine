...
async function waitRuntime(page){
  ...
  await page.waitForFunction(()=>{
    ...
    return Boolean(document.querySelector("iframe"))||/Erro:|Erro ao carregar/i.test(text);
...
async function advanceOneStep(page,device){
  ...
  check(accepted!==false,`M1-12 ${device}: Host recusou avanço pronto na etapa ${before.event?.id||"n/a"}`);
...
async function runM112(browser,device){
  ...
  try{
    ...
    for(let guard=0;guard<20;guard++){
      ...
      if(state.event?.id==="en2-m1-12-drag-drop") break;
      check(state.session?.completed!==true,`M1-12 ${device}: módulo terminou antes da etapa dedicada`);
      await advanceOneStep(page,device);
    }

    const target=await page.evaluate(()=>({event:window.__DUDUQ_QA_STEP__,session:window.DuduQ.getSession()}));
    check(target.event?.id==="en2-m1-12-drag-drop",`M1-12 ${device}: etapa dedicada não alcançada; etapa=${target.event?.id||"n/a"} índice=${target.session?.stepIndex??"n/a"}`);
...
if(failures.length){console.error("DUDUQ YEAR2 v2.3 BROWSER QA: FAIL");process.exit(1)}
