...
function applyPair(doc,s){
 ...
 const t=s.targets[0],c=t?.localComposition;if(c?.type!=="same-canonical-dog-size-pair"||!safeHttp(c.source))return false;
 ...
 style(doc,"duduq-m05-pair-style",`.duduq-m05-pair{width:100%;min-height:170px;display:flex;align-items:flex-end;justify-content:center;gap:clamp(10px,3vw,42px);overflow:hidden}.duduq-m05-pair img{object-fit:contain;display:block;max-height:160px;width:auto}.duduq-m05-pair .small{height:88px}.duduq-m05-pair .big{height:154px}@media(max-width:520px){.duduq-m05-pair{min-height:132px}.duduq-m05-pair .small{height:68px}.duduq-m05-pair .big{height:116px}}`);
 ...
 const pair=doc.createElement("span");pair.className="duduq-m05-pair";pair.setAttribute("role","img");pair.setAttribute("aria-label",String(t.alt||"Dois cachorros iguais, um grande e um pequeno"));
 for(const size of ["big","small"]){const img=doc.createElement("img");img.className=size;img.src=c.source;img.alt="";img.setAttribute("aria-hidden","true");pair.appendChild(img)}
 head.appendChild(pair);root.setAttribute("data-m05-local-composition","same-dog-size-pair");state.pairApplied+=1;state.lastQuestionId=String(s.id);return true;
}
function targetId(target){const a=String(target.getAttribute("aria-label")||"");const m=a.match(/alvo\s+([ABC])(?:\b|$)/i);return m?m[1].toUpperCase():String(target.dataset?.targetId||"").toUpperCase()}
function applyPersonPet(doc,s){
 ...
 const items=new Map(s.items.map(i=>[String(i.id||"").toUpperCase(),i]));const targets=[...doc.querySelectorAll(".duduq-ts-target")];if(targets.length!==3)return false;
...
window.M05VisualComposition=Object.freeze({version:"1.0.0-m05-local",getState:()=>({...state}),scan});
})();