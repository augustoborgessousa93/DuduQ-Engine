...
function applyPersonPet(doc,s){
 if(!moduleOK()||String(s?.id||"")!==PERSON_PET_Q||s?.mode!=="audio-to-image"||!Array.isArray(s.items)||s.items.length!==3)return false;const items=new Map(s.items.map(i=>[String(i.id||"").toUpperCase(),i])),targets=[...doc.querySelectorAll(".duduq-ts-target")];if(targets.length!==3||["A","B","C"].some(id=>!items.has(id)))return false;for(const id of ["A","B","C"]){const item=items.get(id),hay=String(item.accessibleLabel||item.semanticLabel||item.alt||"").toLowerCase();if(!hay.includes(PETS[id].needle))return false}
