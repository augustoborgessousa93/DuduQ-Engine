...
html[data-duduq-m03-question="${Q09}"] .duduq-ts-target-shell{
 background:linear-gradient(145deg,color-mix(in srgb,var(--accent) 72%,#fff) 0 18%,var(--accent) 42% 76%,color-mix(in srgb,var(--accent) 78%,#111827) 100%)!important;
 border:4px solid color-mix(in srgb,var(--accent) 58%,#172033)!important;
 box-shadow:0 6px 0 color-mix(in srgb,var(--accent) 62%,#172033),0 14px 24px rgba(31,65,99,.14),inset 0 3px 0 rgba(255,255,255,.72)!important;
}
...
`);
 ...
 for(const target of targets){const accent=String(target.style.getPropertyValue("--accent")||"").trim().toLowerCase();const name=expected.get(accent);if(!name)return false;target.setAttribute("aria-label",`Alvo visual: ${name}`);target.setAttribute("data-m03-swatch","true");}
 state.q09Applied+=1;state.lastQuestionId=Q09;return true;
}
function applyQ10(doc,s){
 if(!moduleOK()||s?.id!==Q10||s?.mode!=="single-choice"||!Array.isArray(s?.targets)||s.targets.length!==1)return false;
 const target=s.targets[0];if(target.capacity!==1||String(target.alt||"").toLowerCase()!=="três réguas")return false;
 ...
 root.querySelectorAll("img[data-m03-ruler-copy]").forEach(node=>node.remove());
 original.alt="três réguas";original.setAttribute("data-m03-ruler-original","true");
 for(let i=0;i<2;i+=1){const clone=original.cloneNode(false);clone.src=src;clone.alt="";clone.setAttribute("aria-hidden","true");clone.setAttribute("role","presentation");clone.setAttribute("data-m03-ruler-copy",String(i+1));clone.removeAttribute("data-m03-ruler-original");original.parentNode.insertBefore(clone,original.nextSibling);}
 const head=original.parentElement;head?.setAttribute("data-m03-ruler-set","true");root.setAttribute("aria-label","Contexto visual: três réguas");doc.documentElement.setAttribute("data-duduq-m03-question",Q10);
...
window.M03VisualComposition=Object.freeze({version:"1.0.0-m03-local",getState:()=>({...state}),scan});
})();