...
RC1 contains **55 candidate interaction transformations** across M01–M06. Items that cannot be represented safely are not forced: they remain in their original mechanic and are recorded in `gamificationDiversityAudit.skipped` with `REPRESENTATION_NOT_SAFE`.
...
- **M05**: alternate image→audio Matching with audio→highlighted-body Target Shooter; retain Drag & Drop for commands/quantity relations.
...
- [ ] Confirm each module still contains all 15 original IDs in the same order.
- [ ] Confirm all original answers remain correct after mechanic conversion.
- [ ] Confirm no activity requires autonomous English reading to answer.
- [ ] Confirm Matching image→audio exposes four playable audio choices and only the original answer forms the correct pair.
- [ ] Confirm Matching audio→image accepts one correct connection while distractor images remain unpaired.
- [ ] Confirm Bubble Pop number items speak the original English word and display numeral symbols only.
- [ ] Confirm Target Shooter uses large, low-speed image targets and does not introduce motor difficulty as the main source of error.
- [ ] Confirm error feedback still permits another attempt and audio replay without penalty.
...
- [ ] Confirm the approved R143 Drag & Drop visual baseline is not changed by this RC.
