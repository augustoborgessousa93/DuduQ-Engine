...
(function () {
  ...
  if (window.DuduQRouter?.version === "1.0.1") return;
  ...
  /*
   * Contrato explícito do Drag & Drop data-driven.
   *
   * O Schema canônico não preserva payload na questão normalizada,
   * então esta capacidade precisa ser analisada no rawQuestion antes
   * da normalização. O guard espelha o mínimo que buildStage() da
   * mecânica exige para montar uma associação:
   * - items e targets não vazios;
   * - pelo menos um item obrigatório;
   * - cada item obrigatório aponta para um targetId existente;
   * - distratores required=false podem ficar sem targetId.
   */
  function analyzeStructuredDragDropPayload(rawQuestion) {
    ...
    const requiredItemsHaveValidTargets =
  ...
  function analyzeQuestion(
    ...
    const structuredDragDropPayload =
      analyzeStructuredDragDropPayload(
        rawQuestion
      );
    ...
    return {
      ...
      hasStructuredDragDropPayload:
        structuredDragDropPayload.valid,

      structuredDragDropPayload,
  ...
  function evaluate(
    ...
    const dragDropDataDrivenSingle =
      profile.id ===
        "drag-drop" &&
      analysis.answerType ===
        "single" &&
      analysis.requestedMechanic ===
        "drag-drop" &&
      analysis.hasStructuredDragDropPayload ===
        true;
    ...
    if (
      ...
      add(
        30,
        "single autorizado por payload Drag & Drop data-driven válido"
      );
