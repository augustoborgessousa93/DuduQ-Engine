/* =========================================================
   ...
   Candidate Release 1.0.2 — Router Compatibility Fix

   Objetivos:
   - registrar Word Slash de forma síncrona no DuduQ Host;
   - preservar o runtime Word Slash 1.0.0 como base imutável;
   - receber conteúdo pelo contrato padrão do Engine via metadata.wordSlash;
   - aceitar palavras, imagens, cores e categorias;
   - declarar corretamente ao Router os tipos e mídias suportados;
...
(function () {
  ...
  const RELEASE_PATH = "/engine/releases/mechanics/word-slash/1.0.2/";
  ...
  function mount({ container, payload, context = {}, onComplete }) {
    ...
    const runtimeUrl = getEngineBase() + RELEASE_PATH + "DUDUQ_WORD_SLASH.html?engineAdapter=" + encodeURIComponent(VERSION);
