/* DUDUQ WORD SLASH 1.0.11 — Target Shooter visual parity at source */
(function () {
  "use strict";
  ...
  const RUNTIME = "/engine/releases/mechanics/word-slash/1.0.11/DUDUQ_WORD_SLASH.html";
  ...
  const upperInstruction = (v, f) => text(v, f).toLocaleUpperCase("pt-BR");
  ...
  function required(html, from, to, label) {
    if (!html.includes(from)) throw new Error(`[DuduQ Word Slash 1.0.11] Marcador ausente: ${label}.`);
  ...
  function visualSkin(html) {
    const css=`<style id="duduq-word-slash-1-0-11-target-parity">
/* 1.0.11 — transplante direto do Target Shooter 1.0.21 para a casca visual. */
...
/* Enunciado: mesma tipografia efetiva do Target Shooter. */
...
html body #root .duduq-engine-stage .duduq-ws-instruction h2{
  margin:0!important;min-width:0!important;color:#075AB8!important;
  font-family:Fredoka,Nunito,ui-rounded,system-ui,sans-serif!important;
  font-size:clamp(18px,2vw,28px)!important;font-weight:800!important;
  line-height:1.16!important;letter-spacing:0!important;text-align:center!important;
...
/* Arena/subcard: mesmas dimensões estruturais do Target Shooter 1.0.21. */
...
/* Remove apenas a apresentação visual do texto inferior; role=status continua no DOM para acessibilidade. */
...
@media(prefers-reduced-motion:reduce){
  ...
  function mount({container,payload,context={},onComplete}) {
    ...
    }).catch(e=>{console.error("[DuduQ Word Slash 1.0.11]",e);if(!dead)container.textContent="Erro ao preparar a atividade Word Slash.";});
