...
(function () {
  ...
  /*
   * CANARY 080 â€” RUNTIME SELF RESOLVE
   *
   * Captura a URL do adapter enquanto document.currentScript ainda aponta
   * para o arquivo correto. No mount, o runtime sera resolvido como irmao
   * do adapter atual, eliminando para sempre o hardcode de releases antigas.
   */
  const SMART_ADAPTER_SCRIPT_URL =
    document.currentScript?.src || "";

  const __DUDUQ_SMART_RUNTIME_SELF_RESOLVE_080__ = true;

  function resolveOwnRuntimeUrl() {
    let runtimeUrl = "";

    if (SMART_ADAPTER_SCRIPT_URL) {
      try {
        const url = new URL(
          "DUDUQ_SMART_SENTENCE.html",
          SMART_ADAPTER_SCRIPT_URL
        );
  ...
  function stageFromQuestion(question, index) {
    ...
    return {
      ...
      interaction: {
        type: "tap",
        allowRemove: true,
        shuffleOptions: config.options.length > 1
      },
      feedback: {
        success: asString(
  ...
  function mount({
    ...
    /* CANARY 051:
       Ponte explicita iframe -> adapter -> Host.
       O CustomEvent interno continua existindo, mas a conclusao nao depende
       mais dele atravessar corretamente o limite do iframe. */
    const smartCompleteMessageHandler = function (event) {
      if (destroyed || completed) return;
      if (event.source !== iframe.contentWindow) return;
      if (!event.data || event.data.type !== "DUDUQ_SMART_SENTENCE_COMPLETE") {
        return;
      }
    ...
    const runtimeUrl =
      resolveOwnRuntimeUrl();

    console.info(
      "[DuduQ Smart Sentence] Canary 080 runtime self-resolve",
      {
        adapterRelease: VERSION,
        adapterScript: SMART_ADAPTER_SCRIPT_URL,
