/* =========================================================
   DUDUQ MECHANIC — SMART SENTENCE 4.0.3
   TARGET SHOOTER SHELL / FEEDBACK + BRANDING / MODULAR LANGUAGE ENGINE
...
.duduq-smart-ts-feedback {
  ...
  box-shadow: 0 4px 0 rgba(180,202,219,.66);
  color: #16375B;
  font-size: 13px;
  font-weight: 900;
  line-height: 1.2;
...
.duduq-smart-ts-feedback[data-state="success"] {
  border-color: #8AD44C;
  background: #F2FFE8;
  color: #23641E;
  box-shadow: 0 4px 0 #AAD88A;
}

.duduq-smart-ts-feedback[data-state="retry"] {
  border-color: #F0B387;
  background: #FFF3EA;
  color: #8E4720;
  box-shadow: 0 4px 0 #E7BF9F;
}

.duduq-smart-ts-feedback[hidden] {
  display: none;
}
...
(function () {
  ...
  function adaptContent(content) {
    ...
    if (content.mechanicVersion !== "4.0.3") {
      throw new Error(
        "Conteúdo " + content.id +
        " usa smart-sentence@" + content.mechanicVersion +
        "; runtime disponível: 4.0.3."
      );
    }
  ...
  function SmartSentenceTargetShell(props) {
    ...
    var feedbackMessage = feedbackState === "success"
      ? (question.feedback && question.feedback.success) || "Muito bem!"
      : feedbackState === "retry"
        ? (question.feedback && question.feedback.retry) || "Tente novamente."
        : "";
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              "p",
              {
                className: "duduq-smart-ts-feedback",
                "data-state": feedbackState === "success" ? "success" : "retry",
                hidden: feedbackState === "idle"
              },
              feedbackState === "success"
                ? "✅ " + feedbackMessage
                : feedbackState === "retry"
            ...
            React.createElement(
              "p",
              {
                className:
                  "duduq-smart-ts-hint-card",
                hidden: !currentHint || feedbackState !== "retry"
              },
              currentHint
                ? "💡 " + currentHint
                : ""
  ...
  function patchHeaderDuduQBrand() {
    try {
      var header =
        document.querySelector('.duduq-engine-header') ||
      ...
      var first = header.firstElementChild || header.firstChild;
      if (first) {
        header.insertBefore(badge, first);
      } else {
  ...
  try {
    ...
    */
    var mechanics = {
      "smart-sentence@4.0.3": registration
    };
    ...
    window.setTimeout(patchHeaderDuduQBrand, 0);
    window.setTimeout(patchHeaderDuduQBrand, 300);
    window.setTimeout(patchHeaderDuduQBrand, 900);
