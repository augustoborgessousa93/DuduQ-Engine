/* =========================================================
   DUDUQ MECHANIC — SMART SENTENCE 3.0.0
   MATCHING SHELL / MODULAR LANGUAGE ENGINE
...
/* ============================================================
   SMART SENTENCE 3.0.0 — SOMENTE CONTEUDO DO CARD PRINCIPAL
...
(function () {
    ...
    try {
      if (window.parent && window.parent !== window) {
        window.parent.postMessage(
          Object.assign({ type: type, version: "3.0.0" }, detail || {}),
          "*"
        );
      }
    } catch (_) {}
  ...
  function adaptContent(content) {
    ...
    if (content.mechanicVersion !== "3.0.0") {
      throw new Error(
        "Conteudo " + content.id +
        " usa smart-sentence@" + content.mechanicVersion +
        "; runtime disponivel: smart-sentence@3.0.0."
      );
    }
  ...
  try {
    ...
    var mechanics = Object.assign(
      ...
      api.MATCHING_RUNTIME_REGISTRY,
      {
        "smart-sentence@3.0.0": registration
      }
    );
