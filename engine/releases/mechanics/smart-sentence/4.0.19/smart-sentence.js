/* =========================================================
   DUDUQ MECHANIC — SMART SENTENCE 4.0.19
...
/* Remove completamente os círculos decorativos herdados do cenário. */
.duduq-ts-arena[data-smart-sentence-shell="true"] .duduq-ts-ambient::before,
.duduq-ts-arena[data-smart-sentence-shell="true"] .duduq-ts-ambient::after,
.duduq-ts-arena[data-smart-sentence-shell="true"] .duduq-smart-ts-stage::before {
  content: none !important;
  display: none !important;
}

.duduq-ts-arena[data-smart-sentence-shell="true"] .duduq-ts-cloud,
.duduq-ts-arena[data-smart-sentence-shell="true"] .duduq-ts-sparkle {
  display: none !important;
}
...
.duduq-smart-ts-stage {
  position: absolute;
  z-index: 4;
  inset: clamp(12px, 2vh, 20px) clamp(14px, 2vw, 24px) clamp(24px, 3.2vh, 34px);
  min-width: 0;
  min-height: 0;
  display: flex;
...
.duduq-smart-ts-stage::before {
  content: "";
  position: absolute;
  top: clamp(20px, 3vh, 34px);
  right: clamp(10%, 12vw, 16%);
...
.duduq-smart-ts-slot[data-correct="true"] {
  border-color: #58CC02;
  background: #EFFFE7;
  color: #1B5E20;
  box-shadow:
    0 4px 0 #359500,
    0 0 0 4px rgba(88,204,2,.12);
}

.duduq-smart-ts-slot[data-retry="true"] {
  border-style: solid;
  border-color: #F08A68;
  background: #FFF4EF;
  color: #9D3823;
...
.duduq-smart-ts-confirm {
  ...
  box-shadow:
    0 4px 0 #07549F,
    0 8px 14px rgba(7,84,159,.14),
    inset 0 2px 0 rgba(255,255,255,.32);
}

.duduq-smart-ts-confirm:hover:not(:disabled),
.duduq-smart-ts-clear:hover:not(:disabled) {
...
.duduq-smart-ts-confirm[data-pressed="true"] {
  ...
  box-shadow:
    0 0 0 #07549F,
    0 2px 4px rgba(7,84,159,.10),
    inset 0 1px 0 rgba(255,255,255,.22);
}

.duduq-smart-ts-clear:active:not(:disabled),
.duduq-smart-ts-clear[data-pressed="true"] {
  transform: translateY(4px) scale(.992);
  box-shadow: 0 0 0 #B4CADB;
}
...
.duduq-smart-ts-choice[data-retry="true"] {
  border-color: #F08A68;
  background: #FFF4EF;
  color: #9D3823;
  box-shadow:
...
(function () {
  ...
  function adaptContent(content) {
    ...
    if (content.mechanicVersion !== "4.0.19") {
      throw new Error(
        "Conteúdo " + content.id +
        " usa smart-sentence@" + content.mechanicVersion +
        "; runtime disponível: 4.0.19."
      );
    }
  ...
  function SmartSentenceTargetShell(props) {
    ...
    var stateRetryVisual = useState(false);
    var retryVisual = stateRetryVisual[0];
    var setRetryVisual = stateRetryVisual[1];

    var stateRetryLocked = useState(false);
    var retryLocked = stateRetryLocked[0];
    var setRetryLocked = stateRetryLocked[1];

    var statePressedAction = useState("");
    var pressedAction = statePressedAction[0];
    var setPressedAction = statePressedAction[1];
    ...
    var retryClearTimerRef = useRef(null);
    var pressedActionTimerRef = useRef(null);
    ...
    useEffect(function () {
      ...
      if (retryClearTimerRef.current) {
        window.clearTimeout(retryClearTimerRef.current);
        retryClearTimerRef.current = null;
      }
      if (pressedActionTimerRef.current) {
        window.clearTimeout(pressedActionTimerRef.current);
        pressedActionTimerRef.current = null;
      }
    }, [presentationKey]);
    ...
    useEffect(function () {
      ...
      if (feedbackState === "retry" && previous !== "retry") {
        ...
        retryClearTimerRef.current = window.setTimeout(function () {
          setSequence([]);
          setSlots(initialSlots);
          setSelectedUid("");
          setAnnouncement("Resposta incorreta. Tente novamente.");
          setRetryVisual(false);
          setRetryLocked(false);
          retryClearTimerRef.current = null;
        }, 520);
      }
    }, [feedbackState, initialSlots]);
    ...
    var releasePressedAction = useCallback(function (delay) {
      if (pressedActionTimerRef.current) {
        window.clearTimeout(pressedActionTimerRef.current);
      }

      pressedActionTimerRef.current = window.setTimeout(function () {
        setPressedAction("");
        pressedActionTimerRef.current = null;
    ...
    var addToken = useCallback(function (uid) {
      if (disabled || feedbackState === "success" || retryLocked) return;
    ...
    var removeToken = useCallback(function (uid) {
      if (
        disabled ||
        (feedbackState === "success" || retryLocked) ||
            ...
            React.createElement(
              ...
              React.createElement(
                "button",
                {
                  ...
                  className:
                    "duduq-smart-ts-clear",
                  "data-pressed": pressedAction === "clear" ? "true" : "false",
                  disabled:
                    !hasAnswer ||
                    disabled ||
                    feedbackState === "success" ||
                  ...
                  onPointerDown: function () {
                    if (!(!hasAnswer || disabled || feedbackState === "success" || retryLocked || question.interaction.remove === false)) {
                      setPressedAction("clear");
                    }
                  },
                  onPointerUp: function () {
                    releasePressedAction(120);
                  },
                  onPointerCancel: function () {
                    releasePressedAction(0);
                  },
                  onBlur: function () {
                    releasePressedAction(0);
                  },
                  onClick: function () {
                    releasePressedAction(140);
                    clearAnswer();
                  }
                },
                "Limpar"
              ...
              React.createElement(
                "button",
                {
                  ...
                  className:
                    "duduq-smart-ts-confirm",
                  "data-pressed": pressedAction === "confirm" ? "true" : "false",
                  disabled:
                    !ready ||
                    disabled ||
                    feedbackState === "success" ||
                    retryLocked,
                  onPointerDown: function () {
                    if (!(!ready || disabled || feedbackState === "success" || retryLocked)) {
  ...
  try {
    ...
    */
    var mechanics = {
      "smart-sentence@4.0.19": registration
    };
    var mascotAssets = {
      idle: {
