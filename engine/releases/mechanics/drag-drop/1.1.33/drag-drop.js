/* =========================================================
   ...
   Canonical Production Release 1.1.33
...
(function () {
  ...
  const RELEASE_PATH =
    "/engine/releases/mechanics/drag-drop/1.1.33/";

  if (!window.DuduQ) {
    console.error(
      "[DuduQ Drag & Drop 1.1.33] duduq-host.js precisa ser carregado antes."
    );
    return;
  }
  ...
  function mount({
    ...
    if (!container) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.33] Container não informado."
      );
    }
    ...
    if (!serializableSchema) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.33] Falha ao serializar conteúdo."
      );
    }
    ...
    fetch(runtimeUrl)
      ...
      .catch(function (error) {
        console.error(
          "[DuduQ Drag & Drop 1.1.33] Falha ao preparar runtime:",
          error
        );
