/* =========================================================
   ...
   Clean Production Release 1.1.0
...
(function () {
  ...
  const RELEASE_PATH =
    "/engine/releases/mechanics/drag-drop/1.1.0/";

  if (!window.DuduQ) {
    console.error(
      "[DuduQ Drag & Drop 1.1.0] duduq-host.js precisa ser carregado antes."
    );
    return;
  }
  ...
  function mount({
    ...
    if (!container) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.0] Container não informado."
      );
    }
    ...
    if (!serializableSchema) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.0] Falha ao serializar conteúdo."
      );
    }
    ...
    function handleMessage(event) {
      ...
      if (
        type ===
        "duduq:mechanic:schema-accepted"
      ) {
        schemaAccepted = true;
        return;
      }
      ...
      if (
        ...
        onComplete?.({
          ...
          lessonId:
            lesson.id ||
            serializableSchema.lessonId,
          lessonVersion:
            lesson.version ||
            serializableSchema.version,
          totalStages:
            lesson.totalStages ||
            serializableSchema
              .stages.length,
      ...
      if (
        type ===
        "duduq:mechanic:error"
      ) {
        console.error(
          "[DuduQ Drag & Drop 1.1.0] Runtime:",
          data.payload || data
        );
      }
    }
  ...
  console.info(
    "[DuduQ] Drag & Drop CLEAN registrado:",
    VERSION
  );
})();
