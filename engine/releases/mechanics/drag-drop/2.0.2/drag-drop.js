...
(function () {
  ...
  const RELEASE_PATH = "/engine/releases/mechanics/drag-drop/2.0.2/";

  if (!window.DuduQ || typeof window.DuduQ.registerMechanic !== "function") {
    console.error("[DuduQ Drag & Drop 2.0.2] duduq-host.js precisa ser carregado antes.");
    return;
  }
  ...
  function replaceConfig(html, config) {
    ...
    if (start < 0) throw new Error("[Drag & Drop 2.0.2] Configuração do shell Target Shooter não encontrada.");
    const bodyStart = start + marker.length;
    const end = html.indexOf("</script>", bodyStart);
    if (end < 0) throw new Error("[Drag & Drop 2.0.2] Fechamento da configuração não encontrado.");
  ...
  function mount({ container, payload, context = {}, onComplete }) {
    if (!container) throw new Error("[DuduQ Drag & Drop 2.0.2] Container não informado.");

    const config = buildRuntimeConfig(payload, context);
    const serializable = clone(config);
    if (!serializable) throw new Error("[DuduQ Drag & Drop 2.0.2] Conteúdo não pôde ser serializado.");
    ...
    iframe.addEventListener("load", () => {
      ...
      try {
        ...
        console.warn("[DuduQ Drag & Drop 2.0.2] Não foi possível sincronizar o chrome do Target Shooter.", error);
      }
    });
    ...
    fetch(runtimeUrl)
      ...
      .catch((error) => {
        console.error("[DuduQ Drag & Drop 2.0.2] Falha ao preparar runtime:", error);
