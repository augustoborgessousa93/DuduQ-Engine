/* =========================================================
   ...
   Canonical Production Release 1.1.4
...
(function () {
  ...
  const VERSION = "1.1.4";
  const RUNTIME_VERSION = "2.3.0";
  const BRIDGE_VERSION = "1.0.0";
  const RELEASE_PATH =
    "/engine/releases/mechanics/drag-drop/1.1.4/";

  if (!window.DuduQ) {
    console.error(
      "[DuduQ Drag & Drop 1.1.4] duduq-host.js precisa ser carregado antes."
    );
    return;
  }
  ...
  function mount({
    ...
    if (!container) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.4] Container não informado."
      );
    }
    ...
    if (!serializableSchema) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.4] Falha ao serializar conteúdo."
      );
    }
