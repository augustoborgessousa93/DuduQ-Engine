/* =========================================================
   ...
   Canonical Production Release 1.1.8
   Contrato externo: DuduQ Schema / answer pairs|sequence
   Runtime universal no shell Target Shooter: 2.7.0
   ========================================================= */
...
(function () {
  ...
  const VERSION = "1.1.8";
  const RUNTIME_VERSION = "2.7.0";
  const BRIDGE_VERSION = "1.0.0";
  const RELEASE_PATH =
    "/engine/releases/mechanics/drag-drop/1.1.8/";

  if (!window.DuduQ) {
    console.error(
      "[DuduQ Drag & Drop 1.1.8] duduq-host.js precisa ser carregado antes."
    );
    return;
  }
  ...
  function mount({
    ...
    if (!container) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.8] Container não informado."
      );
    }
    ...
    if (!serializableSchema) {
      throw new Error(
        "[DuduQ Drag & Drop 1.1.8] Falha ao serializar conteúdo."
      );
    }
    ...
    fetch(runtimeUrl)
      ...
      .catch(function (error) {
        console.error(
          "[DuduQ Drag & Drop 1.1.8] Falha ao preparar runtime:",
          error
        );
