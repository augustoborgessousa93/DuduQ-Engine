...
(function () {
  ...
  const RELEASE_PATH = "/engine/releases/mechanics/drag-drop/2.0.10/";

  if (!window.DuduQ || typeof window.DuduQ.registerMechanic !== "function") {
    console.error("[DuduQ Drag & Drop 2.0.10] duduq-host.js precisa ser carregado antes.");
    return;
  }
  ...
  function mount({ container, payload, context = {}, onComplete }) {
    if (!container) throw new Error("[DuduQ Drag & Drop 2.0.10] Container não informado.");

    const config = buildRuntimeConfig(payload, context);
    const serializable = clone(config);
    if (!serializable) throw new Error("[DuduQ Drag & Drop 2.0.10] Conteúdo não pôde ser serializado.");
    ...
    iframe.addEventListener("load", () => {
      ...
      try {
        ...
        console.warn("[DuduQ Drag & Drop 2.0.10] Não foi possível sincronizar o chrome do Target Shooter.", error);
      }
    });
    ...
    fetch(runtimeUrl)
      ...
      .catch((error) => {
        console.error("[DuduQ Drag & Drop 2.0.10] Falha ao preparar runtime:", error);
