...
(function () {
  ...
  const VERSION = "1.2.9";
  const SHELL_CSS = "/engine/releases/mechanics/bubble-pop/1.0.27/bubble-pop-shell.css";
  ...
  function mount({
    ...
    iframe.src =
      engineBase +
      "/engine/releases/mechanics/bubble-pop/1.0.27/DUDUQ_BUBBLE_POP.html?" +
      params.toString();
    ...
    function installSubcardParityStyles() {
      try {
        ...
        if (iframeDocument.getElementById("duduq-bubble-pop-shell-parity-1-0-27")) return;

        const stylesheet = iframeDocument.createElement("link");
        stylesheet.id = "duduq-bubble-pop-shell-parity-1-0-27";
