...
try{
  for(let year=1;year<=2;year++)for(let module=1;module<=6;module++){
    ...
    try{
      const mm=String(module).padStart(2,"0"),r=await page.goto(`${BASE}/content/english/year-${year}/module-${mm}/?qa=rendered-media-audit`,{waitUntil:"domcontentloaded",timeout:35000});assert(r?.ok(),`Y${year} M${mm}: HTTP ${r?.status()}`);await page.waitForFunction(()=>window.DUDUQ_ENGINE_READY===true,null,{timeout:35000});
      ...
      await page.waitForFunction(()=>{const s=window.DuduQ?.getSession?.(),f=document.querySelector("iframe");return Boolean(s&&!s.transitioning&&f?.srcdoc&&window.DuduQTransition?.getState?.()==="idle")},null,{timeout:35000});
      ...
      while(Date.now()<deadline&&guard++<total+2){
        ...
        const frame=page.frames().find(f=>f!==page.mainFrame()&&f.url()==="about:srcdoc");assert(frame,`Y${year} M${mm} step ${s?.stepIndex}: iframe ausente`);
        await frame.waitForLoadState("domcontentloaded").catch(()=>{});await page.waitForTimeout(120);
        const media=await frame.evaluate(()=>{
          ...
          return {imgs,styledDataImages:styled.length,text:String(document.body?.innerText||"").slice(0,300)};
        });
        ...
        seen.push({step:s.stepIndex,images:media.imgs.filter(x=>x.visible).length,styledDataImages:media.styledDataImages});
        const accepted=await page.evaluate(()=>window.DuduQ.next({qa:"rendered-media-audit"}));assert(accepted!==false,`Y${year} M${mm}: next rejeitado ${s.stepIndex}`);
