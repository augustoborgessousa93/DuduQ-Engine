...
function sourceAnswerMismatch(q){
  const src=q?.metadata?.sourceAnswer;
  if(src==null||text(q?.answer?.type).toLowerCase()!=="single")return false;
  const expected=text(typeof src==="object"?(src.id??src.value??src.text):src);const actual=text(q?.answer?.value);
  if(!expected||!actual)return false;
  const alt=(q?.alternatives||[]).find(a=>text(a?.id)===actual);
  return expected!==actual&&expected.toLowerCase()!==text(alt?.text).toLowerCase();
}
...
try{
  for(let year=1;year<=2;year++)for(let module=1;module<=6;module++){
    ...
    try{
      const mm=String(module).padStart(2,"0");const res=await page.goto(`${BASE}/content/english/year-${year}/module-${mm}/?qa=pedagogy-audit`,{waitUntil:"domcontentloaded",timeout:35000});
      ...
      for(const {a,q} of questions){
        ...
        if(!answerIntegrity(q))add("P0","PED-11",year,module,id,"answer contract inválido");
        if(sourceAnswerMismatch(q))add("P0","PED-11",year,module,id,"sourceAnswer diverge do answer atual");
...
const byYear={};for(const year of [1,2])byYear[year]={P0:findings.filter(f=>f.year===year&&f.severity==="P0").length,P1:findings.filter(f=>f.year===year&&f.severity==="P1").length};
console.log(JSON.stringify({status:"AUDIT_COMPLETE",modules:modules.length,items:modules.reduce((n,m)=>n+m.items,0),byYear,findings,modules},null,2));
