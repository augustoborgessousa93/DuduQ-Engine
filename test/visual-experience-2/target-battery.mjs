...
const report={battery:1,mechanic:'target-shooter',base:'00c1ba10042eb91fbc98d86f1cb8f33f640263ef',
  startedAt:new Date().toISOString(),cases:[],screenshots:[],limitations:[],status:'RUNNING'};
...
async function inspect(frame){
  return frame.evaluate(()=>{
    ...
    return {width:w,height:h,dpr:devicePixelRatio,overflow:Math.max(document.documentElement.scrollWidth,document.body.scrollWidth)-w,
      clipped,images,roots,arena:arena&&{width:arena.width,height:arena.height},
      focusable:controls.length,motion:matchMedia('(prefers-reduced-motion: reduce)').matches};
  });
}

try{
  // Grounding captures use the same real adapter and official questions.
  ...
  const scenarios=[...dimensions.map(([width,height])=>({width,height,dpr:1})),
    ...[[1366,900],[768,1024],[390,844]].map(([width,height])=>({width,height,dpr:2}))];
  for(const {width,height,dpr} of scenarios){
    const key=`${width}x${height}-dpr${dpr}`;
    const result={key,failures:[],checks:{},errors:[]};report.cases.push(result);
    ...
    try{
      ...
      if(g.overflow>2) failure(result,`Horizontal overflow: ${g.overflow}px`);
      if(g.clipped.length) failure(result,`Controls outside viewport: ${g.clipped.join(', ')}`);
      if(!g.arena||g.arena.width<200||g.arena.height<120) failure(result,'Collapsed arena');
      ...
      await frame.getByRole('button',{name:`Lançar estrela no alvo ${bad.label}`,exact:true}).click();
      ...
      if(result.retryGeometry.clipped.length) failure(result,'Retry controls outside viewport');

      await frame.getByRole('button',{name:`Lançar estrela no alvo ${good.label}`,exact:true}).click();
      ...
      if(result.correctGeometry.clipped.length) failure(result,'Success controls outside viewport');
  ...
  const sweep=await browser.newPage({viewport:{width:1366,height:900},reducedMotion:'reduce'});
  try{
    await sweep.goto(`${base}/test/visual-experience-2/target.html`,{waitUntil:'domcontentloaded'});
    const frame=await frameFor(sweep,'after');report.resize=[];
    for(const width of [390,430,480,540,600,700,768,865,960,1071,1200,1280,1366]){
      await sweep.setViewportSize({width,height:700});
      await frame.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
      const geometry=await inspect(frame);report.resize.push({width,overflow:geometry.overflow,clipped:geometry.clipped});
    }
    report.reducedMotion=await frame.locator('.duduq-mascot-body').first().evaluate(e=>getComputedStyle(e).animationName==='none');
    report.performance=await frame.evaluate(()=>({activeAnimations:document.getAnimations().filter(a=>a.playState==='running').length}));
  }catch(error){report.limitations.push(`Resize/reduced-motion: ${error.message}`);}
  finally{await sweep.close();}
}finally{
  report.status=report.limitations.length===0&&report.cases.length===11&&report.cases.every(c=>c.status==='PASS')&&report.reducedMotion&&
    report.resize?.length===13&&report.resize.every(r=>r.overflow<=2&&!r.clipped.length)?'PASS':'NO-GO';
