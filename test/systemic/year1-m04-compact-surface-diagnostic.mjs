...
async function measure(page,module){
  return await page.evaluate(module=>{
    ...
    const ts=d?.querySelector(".duduq-ts-root"),dd=d?.querySelector(".duduq-dd2-root"),root=dd||ts;
...
try{
 for(const module of [3,4]){
  ...
  try{
   const r=await page.goto(`${BASE}/content/english/year-1/module-0${module}/?qa=compact-surface-${NAME}`,{waitUntil:"domcontentloaded",timeout:35000});assert(r?.ok(),`M0${module}: HTTP ${r?.status()}`);await page.waitForFunction(()=>window.DUDUQ_ENGINE_READY===true,null,{timeout:35000});
   ...
   await page.waitForFunction(()=>{const s=window.DuduQ?.getSession?.(),f=document.querySelector("iframe"),d=f?.contentDocument;return Boolean(s&&!s.transitioning&&!s.completed&&window.DuduQTransition?.getState?.()==="idle"&&d?.body&&(d.querySelector(".duduq-ts-root")||d.querySelector(".duduq-dd2-root")))},null,{timeout:25000});
   if(module===4)await page.waitForFunction(()=>{const d=document.querySelector("iframe")?.contentDocument,t=[...(d?.querySelectorAll(".duduq-ts-target")||[])];return Boolean(d?.querySelector(".duduq-ts-root")&&t.length===3&&t.every(x=>!x.disabled))},null,{timeout:12000});
   const m=await measure(page,module);m.pageErrors=errors;m.critical404=critical404;results.push(m);console.log(`COMPACT_SURFACE ${NAME} M0${module} ${JSON.stringify(m)}`);await fs.writeFile(path.join(OUT,`m0${module}.json`),JSON.stringify(m,null,2));await page.screenshot({path:path.join(OUT,`m0${module}.png`),fullPage:true});
  }finally{await page.close()}
 }
 const m3=results.find(x=>x.module===3),m4=results.find(x=>x.module===4);const summary={viewport:NAME,m03Frame:m3?.iframe?.rect||null,m04Frame:m4?.iframe?.rect||null,m03Mechanic:m3?.mechanic,m04Mechanic:m4?.mechanic,m04Targets:m4?.targets?.map(x=>x.rect)||[],m04FrameDocument:m4?.frameDocument||null,pageErrors:results.flatMap(x=>x.pageErrors||[]),critical404:results.flatMap(x=>x.critical404||[])};console.log(`COMPACT_SURFACE_SUMMARY ${JSON.stringify(summary)}`);await fs.writeFile(path.join(OUT,"summary.json"),JSON.stringify(summary,null,2));
