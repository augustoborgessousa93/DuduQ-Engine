...
async function liveState(page) {
  return page.evaluate(() => {
    ...
    const ttsSafety = w?.__DUDUQ_QA_TTS_SAFETY_STATE__ || null;
    return {
...
export async function runQuestionGuarded({
  ...
  try {
    ...
    if (isTimeout(error)) await closeBrowserBounded(browser, 500);
...
async function clickCardDom(page, id) {
  return page.evaluate((itemId) => {
    ...
    const ttsCallsBefore = Number(w?.__DUDUQ_QA_TTS_SAFETY_STATE__?.calls || 0);
    card.click();
    return { hasAudio, clicked: probe.clicked, ttsCallsBefore };
  }, id);
}

async function clickZoneDom(page) {
  return page.evaluate(() => {
    const d = document.querySelector("iframe")?.contentDocument;
    const zone = d?.querySelector(".duduq-dd2-zone,.duduq-dd2-target[data-dd2-target-id]");
    if (!zone) throw new Error("DD zone not found");
    if (zone.disabled || zone.getAttribute("aria-disabled") === "true" || zone.classList.contains("is-disabled")) {
      throw new Error("DD zone disabled");
    }
    zone.click();
    return true;
  });
...
async function readAudioObservation(page, id) {
  return page.evaluate((itemId) => {
    ...
    return {
      ...
      customPlaying,
      htmlAudioPlaying,
      speechSpeaking: !!(synth?.speaking || synth?.pending),
...
function validateFallback(fallback, id, errorsBefore, pageErrors) {
  if (!fallback?.clicked) throw new Error(`DD ${id}: card click was not observed`);
  if (pageErrors.length !== errorsBefore) {
    throw new Error(`DD ${id}: JavaScript error after card click: ${pageErrors.slice(errorsBefore).join(" | ")}`);
  }
...
export async function clickDdOptionWithAudio(page, id, {
  ...
  guard?.note?.({ phase: `card-click:${id}`, audioState: { mode: "probing" } });
  const click = await clickCardDom(page, id);
  if (!click.clicked) throw new Error(`DD ${id}: DOM click did not dispatch`);

  let mode = "no-audio";
  if (click.hasAudio) {
    const immediate = await safeBounded(() => readAudioObservation(page, id), 900, null);
    const safetyCallObserved = !!(
      immediate?.ttsSafety?.nativeSuppressed &&
      Number(immediate.ttsSafety.calls || 0) > click.ttsCallsBefore
    );

    if (safetyCallObserved) {
      validateFallback(immediate, id, errorsBefore, pageErrors);
      mode = "headless-tts-suppressed";
      guard?.note?.({
        phase: `audio-headless-fallback:${id}`,
        audioState: { mode, ttsSafety: immediate.ttsSafety },
        targetPresent: immediate.targetPresent,
        targetEnabled: immediate.targetEnabled,
        interactionEnabled: immediate.interactionEnabled,
      });
    } else {
      ...
      if (started) {
        ...
        validateFallback(fallback, id, errorsBefore, pageErrors);
        mode = "headless-no-observable-playback";
        guard?.note?.({
          phase: `audio-headless-fallback:${id}`,
          audioState: { mode, speechAvailable: fallback.speechAvailable, ttsSafety: fallback.ttsSafety },
  ...
  guard?.note?.({
    phase: `zone-click:${id}`,
    audioState: critical404.length ? { mode, critical404Present: true } : { mode },
  });
  await clickZoneDom(page);
  return { mode };
}
