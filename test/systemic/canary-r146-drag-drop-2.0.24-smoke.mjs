...
try {
  const response = await page.goto(`${BASE}/content/english/year-1/module-01/?qa=canary-r146-dd224`, {
    waitUntil: "domcontentloaded",
    timeout: 35_000
  });
  assert(response?.ok(), `M01 entry HTTP ${response?.status()}.`);
  ...
  const mount = await page.evaluate(() => {
    ...
    return {
      targetShooter: Boolean(doc?.querySelector(".duduq-ts-root")),
  ...
  assert(mount.targetShooter, "Primeira mecânica M01 não montou pelo Loader.");
  assert(Math.max(0, mount.bodyWidth - mount.viewportWidth) <= 6, "Overflow no mount Loader M01.");
  assert(pageErrors.length === 0, `JS blocker: ${pageErrors.join(" | ")}`);
  assert(critical404.length === 0, `404 crítico: ${critical404.join(" | ")}`);

  await page.screenshot({ path: path.join(OUT, "loader-m01-r146.png"), fullPage: false });
  const report = {
