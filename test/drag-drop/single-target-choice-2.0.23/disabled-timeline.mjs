...
try {
  ...
  while (Date.now() - startedAt < 7000) {
    ...
    const inner = await frame.evaluate(() => {
      ...
      const audio = Array.from(document.querySelectorAll("audio")).map((el) => ({ paused: el.paused, ended: el.ended, currentTime: Math.round((el.currentTime || 0) * 100) / 100 }));
      return {
  ...
  console.log("=== DD2 DISABLED TIMELINE ===");
  ...
  const disabledAfterEnabled = transitions.some((entry, index) => {
    if (entry.buttonDisabled !== true && entry.arenaDisabled !== "true") return false;
    return transitions.slice(0, index).some((prior) => prior.buttonDisabled === false && prior.arenaDisabled !== "true");
  });
  const last = transitions.at(-1) || null;

  if (enabledTransitions.length === 0) {
    throw new Error("DD2 nunca ficou interativo durante a janela de 7s.");
  }
  if (!last || last.buttonDisabled !== false || last.arenaDisabled === "true") {
    throw new Error(`DD2 não terminou a janela em estado interativo: ${JSON.stringify(last)}`);
  }

  console.log(`PASS — timeline capturada; disabledAfterEnabled=${disabledAfterEnabled}; estados=${transitions.length}`);
  await context.close();
} finally {
  await browser.close();
}
