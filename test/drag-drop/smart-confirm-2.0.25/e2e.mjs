...
async function clickPlace(frame,itemId,targetId,touch=false){const i=frame.locator(item(itemId)).first(),z=frame.locator(zone(targetId)).first();if(touch)await i.tap({force:true});else await i.click({force:true});await frame.locator(`${item(itemId)}[data-selected="true"]`).waitFor({state:"visible",timeout:2500});if(touch)await z.tap({force:true});else await z.click({force:true});}
async function keyboardPlace(frame,itemId,targetId){const i=frame.locator(item(itemId)).first(),z=frame.locator(zone(targetId)).first();await i.focus();await i.press("Enter");await frame.locator(`${item(itemId)}[data-selected="true"]`).waitFor({state:"visible",timeout:2500});await z.focus();await z.press("Enter");}
