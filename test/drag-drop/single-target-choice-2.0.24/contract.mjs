...
// Produção congelada na baseline aprovada.
...
// Interação + áudio.
expect(runtime.includes("playSingleTargetSelectionAudio"), "Autoplay de áudio por seleção ausente.");
expect(runtime.includes("< 260"), "Debounce curto de áudio ausente.");
expect(runtime.includes('ctx.playChoiceAudio(ctx.itemMap.get(activeDrag.itemId))'), "Drag não dispara áudio da alternativa.");
...
console.log("PASS — DD 2.0.24 static contract: R143 public frozen + isolated candidate + clean prompt/audio + stable target + selection audio");
